...
class MainFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        if WINDOWS:
            # Scale window size for high DPI devices.
            #
            # This func can be called on all operating systems,
            # but scales only for Windows. If scaled value is
            # bigger than the work area on the display then it
            # will be reduced.

            (_, _, max_width, max_height) = wx.GetClientDisplayRect().Get()
            # noinspection PyUnresolvedReferences
            size = cef.DpiAware.Scale(size)
            if size[0] > max_width:
                size[0] = max_width
...
class ServerThread(threading.Thread):
    ...
    def __init__(self, app):
        self.log = logging.getLogger('spam_application')
    ...
    def run(self):
        self.server.log.info('starting server')
...
def MainClass(object):

    __slot__ = ('app', 'server')
    server = None
    app = None

    def GetClientDisplayRect(self):
        pass
    ...
    def start_server(self):
        self.server.start()
        self.server.log.info('server started')

    def stop_server(self):
        self.server.shutdown()
        self.server.log.info('server stopped')

    def main():
        MainClass.check_versions()
    ...
    def check_versions():
        """Check version."""
        ver = cef.GetVersion()
        ver["python_version"] = platform.python_version()
        ver["architecture"] = platform.architecture()[0]
        ver["pillow_version"] = pillow_version

        print("""[screenshot.py] CEF Python {ver[version]}
[screenshot.py] Chromium {ver[chrome_version]}
[screenshot.py] CEF {ver[cef_version]}
[screenshot.py] Python {ver[python_version]} {ver[architecture]}
[screenshot.py] Pillow {ver[pillow_version]}""".format(ver=ver))
