...
"""
Example of embedding CEF browser using PyQt4, PyQt5 and PySide libraries.
# This example has two widgets: a navigation
# bar and a browser.
#
...
"""
from PySide2.QtWidgets import QWidget, QGridLayout, QFrame, QLineEdit, QPushButton, QMainWindow, QApplication
from PySide2.QtCore import *
from PySide2.QtGui import QIcon, QWindow, QPixmap
...
def check_versions():
    print(f"[qt.py] CEF Python {cef.__version__}")
    print("[qt.py] Python {ver} {arch}".format(
        ver=platform.python_version(), arch=platform.architecture()[0]))

    print(f"[qt.py] PySide2 {PySide2.__version__} (qt {QtCore.__version__})")
...
class NavigationBar(QFrame):
    ...
    def createButton(self, name):
        resources = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                 "resources")
        pixmap = QPixmap(os.path.join(resources, f"{name}.png"))
        icon = QIcon(pixmap)
        button = QPushButton()
        button.setIcon(icon)
